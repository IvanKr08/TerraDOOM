...
namespace ManagedDoom
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill)Ext.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = Ext.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = Ext.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = Ext.Clamp(map, 1, 9);
			}
