...
namespace ManagedDoom.Video
{
    public sealed class AutoMapRenderer
    {
        ...
        public void Render(Player player)
        {
            ...
            renderViewX = (float)Math.Round(zoom * ppu * actualViewX) / (zoom * ppu);
            renderViewY = (float)Math.Round(zoom * ppu * actualViewY) / (zoom * ppu);
            ...
            for (var i = 0; i < am.Marks.Count; i++)
            {
                ...
                screen.DrawPatch(
                    markNumbers[i],
                    (int)(float)Math.Round(pos.X),
                    (int)(float)Math.Round(pos.Y),
                    scale);
            }
