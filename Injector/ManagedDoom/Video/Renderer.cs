...
namespace ManagedDoom.Video
{
    public sealed class Renderer
    {
        ...
        public Renderer(Config config, GameContent content)
        {
            ...
            config.video_gamescreensize = Ext.Clamp(config.video_gamescreensize, 0, MaxWindowSize);
            config.video_gammacorrection = Ext.Clamp(config.video_gammacorrection, 0, MaxGammaCorrectionLevel);
        ...
        public unsafe void Render(Doom doom, byte* destination, Fixed frameFrac)
        {
            if (doom.Wiping)
            {
        ...
        private unsafe void RenderWipe(Doom doom, byte *destination)
        {
            ...
            for (var i = 0; i < wipeBandCount - 1; i++)
            {
                ...
                for (var x = x1; x < x2; x++)
                {
                    var y = (int)(float)Math.Round(y1 + dy * ((x - x1) / 2 * 2));
        ...
        private unsafe void WriteData(uint[] colors, byte *destination) {
            uint *p = (uint*)destination;

            var screenData = screen.Data;
            for (var i = 0; i < screenData.Length; i++) {
                *p = colors[screenData[i]];
                p++;
            }
        }
